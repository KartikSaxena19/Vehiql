...
export async function processCarImageWithAI(file) {
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: "Failed to parseAI response",
      };
    }
  } catch (error) {
...
// Add A Car    --> as it the private route check userid and db
export async function addCar({ carData, images }) {
  try {
    ...
    //Unique folder name for every car
    const carId=uuidv4()
...
try {
  ...
  //while extracting the price from ai it gives string but we need decimal so this function is used in lib
